...
const detalleModalBodega = document.getElementById('detalle-modal');
const detalleModalBodyBodega = document.getElementById('detalle-modal-body');

// Inicializar las funciones de bodega
function initBodega() {
    setupEventListenersBodega();
}

// Configurar event listeners
function setupEventListenersBodega() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('btn-detalle') && 
            e.target.closest('#bodega-panel')) {
            const solicitudId = e.target.getAttribute('data-id');
            showDetalleBodega(solicitudId);
        }
    });
}
...
async function loadBodegaData() {
    try {
        ...
        renderTablaSolicitudesBodega(solicitudes);
    } catch (error) {
...
function renderTablaSolicitudesBodega(solicitudes) {
...
async function showDetalleBodega(solicitudId) {
    try {
        const solicitudes = await window.db.loadSolicitudes();
        ...
        if (solicitud) {
            ...
            detalleModalBodyBodega.innerHTML = `
                <div class="row mb-3">
                    <div class="col-md-6">
            ...
            // Mostrar el modal
            const modal = new bootstrap.Modal(detalleModalBodega);
            modal.show();
        } else {
